...
class TC3(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.report.Select_Sheet_By_Name("3")
        self.page = WebPage()
        self.driver = self.page.Start_Up(CONST.URL, self.browser)


    def test_Excute(self):
        casedirpath = Path(self.reportfilepath).parent / Path("TC3")
        ...
        for i in self.excel.Get_Excution_DataSet("executed"):
            try:
                ...
                self.driver.save_screenshot(str(stepsdirpath) + "/TC3_DataSet_%s_Step1.png" % str(i))
                self.page.Input(self.excel.Get_Value_By_ColName("PW",i,casedirpath),"password")
                self.driver.save_screenshot(str(stepsdirpath) + "/TC3_DataSet_%s_Step2.png" % str(i))
                ...
                if self.page.Verify_Text(self.excel.Get_Value_By_ColName("Assertion", i), "viewport-message-area-body",
                                         r"div#%s div span"):
                    print("success")
                else:
                ...
                time.sleep(2)
                snapshot = str(faildirpath.absolute()) + r"\TC3_fail_on_dataset_%s.png" % str(
                ...
                time.sleep(2)
                snapshot = str(passdirpath.absolute()) + r"\TC3_pass_on_dataset_%s.png" % str(int(self.excel.Get_Value_By_ColName("Data set",i)))
    ...
    def Generate_Final_Report(self):
        ...
        randomfilepath = Path(self.reportfilepath).parent / Path("TC3")
        ...
        for rowindex in range(1, self.excel.Get_Row_Numbers()):
            ...
            self.report.Set_Value_By_ColName(3, "Case No", self.wtrowindex)
